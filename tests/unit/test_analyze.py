...
class TestPostProcess:
    def test_snaps_start_and_end_to_word_boundaries(self, sample_transcript):
    ...
    def test_drops_candidate_shorter_than_minimum_after_snapping(self, sample_transcript, test_settings):
    ...
    def test_clamps_timestamps_to_transcript_duration(self, sample_transcript):
    ...
    def test_filters_invalid_categories(self, sample_transcript):
        candidates = [_candidate(categories=["funny", "made_up_category"])]
        out = _post_process(candidates, sample_transcript)
        assert out[0].categories == ["funny"]

    def test_defaults_to_viral_when_all_categories_invalid(self, sample_transcript):
        candidates = [_candidate(categories=["nonsense"])]
        out = _post_process(candidates, sample_transcript)
        assert out[0].categories == ["viral"]

    def test_ranks_best_score_first(self, sample_transcript):
        candidates = [
...
@pytest.mark.unit
def test_analyze_calls_claude_and_post_processes(sample_transcript, mocker):
